...
async function fetchIQScores () {
    try {
        const [ initialIQ, score1, score2, score3 ] = await Promise.all([
            // fetch initial IQ
            fetch('/api/age').then(response => response.json()),
            // fetch score from test-one
            fetch('/api/test-one').then(response => response.json()),
            // fetch score from test-two
            fetch('/api/test-two').then(response => response.json()),
            // fetch score from test-three
            fetch('/api/test-three').then(response => response.json())
        ]);
